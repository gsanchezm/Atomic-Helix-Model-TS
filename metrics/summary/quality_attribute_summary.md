...
|---|---:|---:|---|---|---|
| Maintainability | average_file_size_loc | 71.7 | loc | ALL | src/**/*.ts |
| Maintainability | cyclomatic_complexity_if_available | NOT_AVAILABLE | complexity | ALL | no AST pass (heuristic unavailable) |
| Maintainability | duplicated_code_percentage | 7.46 | percent | ALL | src/**/*.ts |
| Maintainability | duplicated_loc | 1406 | loc | ALL | src/**/*.ts |
| Maintainability | failure_bucket_coverage_percentage | 100 | percent | ALL | metrics/processed/failure_buckets.csv |
| Maintainability | files_touched_per_change | 2.26 | files | ALL | git log |
| Maintainability | max_file_size_loc | 849 | loc | ALL | src/**/*.ts |
| Maintainability | telemetry_completeness_percentage | NOT_AVAILABLE | percent | ALL | metrics/raw/tool-events/*.jsonl (none) |
...
| Reusability | feature_to_tool_coverage | 0.23 | ratio | ALL | executed (feature,tool) pairs / expected (feature,tool) pairs from tags; NA when tool attribution absent |
| Reusability | locator_contract_reuse_count | 15 | count | ALL | heuristic: *.locators.json under src/core/tests/**/contracts/** |
| Reusability | scenario_reuse_ratio | 0.61 | ratio | ALL | metrics/processed/platform_coverage_matrix.csv |
| Reusability | shared_contract_reuse_count | 28 | count | ALL | heuristic: all contract files under src/core/tests/**/contracts/** |
| Reusability | shared_step_reuse_count | 15 | count | ALL | heuristic: src/core/tests/**/step_definitions/*.ts file count |
...
| Quality Attribute | Metric | Value | Unit | Tool | Source |
|---|---:|---:|---|---|---|
| Reliability | fail_rate | 0.01 | ratio | ALL | metrics/processed/scenario_outcome_history.csv |
| Reliability | fail_to_pass_probability |  | ratio | ALL | metrics/processed/scenario_outcome_history.csv |
| Reliability | flaky_scenario_count | 0 | count | ALL | metrics/processed/scenario_outcome_history.csv |
| Reliability | infrastructure_failure_rate | 0 | ratio | ALL | metrics/processed/failure_buckets.csv (INFRASTRUCTURE_FAILURE / total observations) |
| Reliability | pass_rate | 0.99 | ratio | ALL | metrics/processed/scenario_outcome_history.csv |
...
| Quality Attribute | Metric | Value | Unit | Tool | Source |
|---|---:|---:|---|---|---|
| Performance Efficiency | grpc_or_ipc_latency_ms | NOT_AVAILABLE | ms | ALL | TOM-only overhead |
| Performance Efficiency | job_duration_ms | 39750 | ms | ALL | metrics/raw/run-manifest/*.json (mean per-manifest endedAt - startedAt) |
| Performance Efficiency | p50_scenario_duration_ms | NOT_AVAILABLE | ms | ALL | metrics/processed/scenario_durations.csv (p50 of duration_ms) |
| Performance Efficiency | p95_scenario_duration_ms | NOT_AVAILABLE | ms | ALL | metrics/processed/scenario_durations.csv (p95 of duration_ms) |
| Performance Efficiency | p99_scenario_duration_ms | NOT_AVAILABLE | ms | ALL | metrics/processed/scenario_durations.csv (p99 of duration_ms) |
| Performance Efficiency | proxy_overhead_ms | NOT_AVAILABLE | ms | ALL | TOM-only overhead |
| Performance Efficiency | scenario_duration_ms | NOT_AVAILABLE | ms | ALL | metrics/processed/scenario_durations.csv (mean of duration_ms) |
...
| Performance Efficiency | workflow_duration_ms | 18192000 | ms | ALL | metrics/raw/run-manifest/*.json (max endedAt - min startedAt) |
...
| Observability | missing_run_manifest_count | 3 | count | ALL | metrics/processed |
| Observability | missing_scenario_duration_count | 519 | count | ALL | metrics/processed/scenario_outcome_history.csv |
...
| Observability | telemetry_completeness_percentage | NOT_AVAILABLE | percent | ALL | metrics/raw/tool-events (none) |
| Observability | telemetry_event_count | 0 | count | ALL | metrics/raw/tool-events (none) |
...
| Quality Attribute | Metric | Value | Unit | Tool | Source |
|---|---:|---:|---|---|---|
| Portability | environment_specific_config_count | 13 | count | ALL | src/** (caps\|profile) + *.env profiles |
...
| Portability | platform_specific_code_count | 43 | count | ALL | src/** (android\|ios\|mobile\|web in path) |
| Portability | platform_specific_locator_count | 15 | count | ALL | src/**/*.locators.json |
...
| Portability | successful_tool_count | 2 | count | ALL | metrics/processed/scenario_outcome_history.csv |
...
| Interoperability | oracle_composition_count | 6 | count | ALL | src/core/tests (*.feature tags) |
| Interoperability | oracle_count | 1 | count | ALL | metrics/processed/{api_isolated_results,visual_comparison_results,performance_summary}.csv; tool-events |
...
| Interoperability | tool_count | 2 | count | ALL | metrics/processed/scenario_outcome_history.csv |
