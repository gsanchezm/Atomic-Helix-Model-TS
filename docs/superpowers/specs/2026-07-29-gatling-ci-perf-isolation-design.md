...
- `smoke` keeps its current behavior: starts as soon as `gate-gatling` allows, overlaps freely, gives the fast early signal.
...
| Fence membership | The 12 backend-traffic jobs: `api-smoke`, `e2e-web`, `e2e-web-responsive`, `visual-web`, `visual-web-responsive`, `a11y-web`, `e2e-android`, `e2e-ios`, `e2e-mobilewright`, `security-zap`, `e2e-webdriverio`, `e2e-cypress`. | Membership criterion is "generates traffic against the shared backend". Excluded: `security-mobsf` (static APK analysis, zero backend traffic; if it ever hangs it would pointlessly block perf for up to 6 h), `resolve-omnipizza-release` (GitHub API only), all `gate-*` jobs (no-ops). |
