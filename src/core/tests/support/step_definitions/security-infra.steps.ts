...
When('the live frontend is crawled for exposed vulnerabilities', SCAN_TIMEOUT, async function () {
    await route().runBaselineScan();
});

When("the backend's transport security is inspected", SCAN_TIMEOUT, async function () {
    await route().runTlsCheck();
});

When('the mobile app binaries are statically analyzed for exposed vulnerabilities', SCAN_TIMEOUT, async function () {
    await route().runMobsfScans();
});

Then('the findings are captured for review', function () {
