...
- [x] **New risk found 2026-08-23** — the determinism instrument's two arms are exposed asymmetrically to backend load — the twin performs a real UI login (and UI cart-building) on every one of its N=30×K journey rows, while the atomic arm's precondition is a single API `$S_0$` call. This project has twice this month (2026-08-21 stagger-jitter investigation, 2026-08-22/23 ZAP Path Traversal false-positive investigation) documented that concurrent CI load against the backend produces mid-run 502/503/429 responses, and that job-start jitter alone does not prevent mid-run collisions; the subsequent Render plan upgrade raises the threshold but doesn't remove the structural asymmetry (see §10.1). If that dynamic recurs during the 120-dispatch determinism campaign, it would inflate the twin's measured pass↔fail transition rate for an infrastructure reason, not a method reason — directly confounding the paper's causal claim for exactly the instrument that carries it. (a) **Done this session**: §10.1 threat-to-validity entry added. (b) **Closed**: the campaign orchestrator (build-order step 5, `run-campaign.ts`, commit `342f922`) was built 2026-08-24 already covering both instruments, and enforces an in-flight concurrency check before every dispatch, plus the disclosed `INFRASTRUCTURE_FAILURE`/`likelyInfra`-flagging rule. (c) **Verified, not just built, 2026-08-31**: the 120-dispatch determinism campaign ran to completion (2026-08-29–31) — 0 items flagged `likelyInfra` across all 120 dispatches, alongside the 8-dispatch parallel-safety sweep (2026-08-24), also 0 flagged. The confound did not materialize in either campaign; see §10.1 for the updated status (dormant, not eliminated — the structural request-volume asymmetry itself is unchanged).
...
- [x] Results — the twin suites exist and are verified live on both platforms (see above). The campaign orchestrator (build-order step 5, `run-campaign.ts`/`aggregate-campaign-artifacts.ts`) was built and now drives §9.1 (parallel safety, `ps-2026-campaign`, 8/8 dispatches, populated 2026-08-31), §9.3 (determinism, `det-2026-campaign`, 120/120 dispatches, populated 2026-08-31), §9.4 (portability, populated 2026-08-23), and §9.5 (execution efficiency, N≥10 both platforms as of 2026-08-27/28). **§9.2 (diagnosability) — populated 2026-09-01**, the last remaining gap: the fault-injection harness (2026-08-23 bullet above) was wired into the orchestrator, a 20-dispatch campaign (`diag-2026-campaign`) run to completion, and the blast-radius/localization-accuracy table written with real data, including a root-caused `failure-buckets.ts` classifier bug and a twin coverage gap disclosed rather than hidden. **All five §9 sub-instruments are now populated with real data — no remaining results gap.**
...
actual measurement — is reported in §9.4 (63 LOC, 1 file, spec-forced; 5 LOC plugin-gap, disclosed but
not counted). See `docs/superpowers/specs/2026-07-23-atomic-testing-evaluation-campaign-design.md`
...
**Full `w1`–`w8` sweep complete (`ps-2026-campaign`, 8 dispatches: 2 arms × 4 worker levels, web only —
one dispatch per condition, `cucumber_parallel` set to the worker level).** Dispatched 2026-08-24 by the
campaign orchestrator (the same batch doubled as the execution-efficiency instrument's `w1` pair, §9.5 —
both instruments need the same clean single/multi-worker web dispatches, so the `w1` runs were reused
rather than re-dispatched); read out 2026-08-31 by a dedicated
`scripts/experiments/parallel-safety-table.ts` (`pnpm experiments:parallel-safety-table`), **not**
`measure-reliability.ts`'s per-(tool_name, batch) slice — that slice deliberately pools every `run_index`
within one batch, correct for determinism/efficiency where `run_index` is a repeat of the *same*
condition, but wrong here: parallel-safety repurposes `run_index` as a worker-level *label* (`w1`…`w8`),
and pooling it away would erase exactly the per-worker-level breakdown this table needs. Zero failures in
either arm at any worker level:
...
**On the four atomic runs' workflow-level `failure` conclusion (checked, not assumed).** All four
atomic-web GH runs report a `failure` conclusion at the workflow level while the table above reads
0.00 — confirmed via `gh run view --json jobs` + `--log-failed` (all four, not sampled) that the failing
job in every case is `Visual — Playwright Desktop (Pixelmatch)`, an unrelated `@visual`-tagged job
outside this instrument's scope: its own 95 functional scenarios all passed, and the failure comes
entirely from a separate `visual-gate.js` post-step reporting 50 *missing* baselines (`Missing visual
baseline for ... Run UPDATE_BASELINE ...`) for this run context — a baseline-bootstrap gap, not a
functional regression or a visual diff. `relevantJobNamePrefixes` (`lib/campaign-matrix.ts`) already
excludes this job from what the campaign manifest scores, so it did not affect the table above; disclosed
here because a workflow-level red X next to a 0.00 failure-rate row would otherwise look unexplained.

**A clean null across the entire planned sweep — extending, not revising, §8.4's reframing note.** This
application has no account-keyed backend state for the twin's R2 violation to collide against at any
worker count (§8.4), so the prediction this instrument can actually test is "does concurrent same-account
UI load degrade correctness or surface backend-capacity limits," not a data-collision delta. Zero failures
at every worker level, in both arms, is the expected shape of that already-explained null result — it
generalizes the single `w4` twin data point from the reframing note to the full sweep (plus the atomic
arm, not previously dispatched) without changing the conclusion. **One caveat this dataset cannot
resolve:** each cell is N=1 — one dispatch per worker level, not a repeat series like §9.3's 30 — so "zero
failures" here means "no failure observed in one clean run at that worker level," not a statistically
powered claim that failures never occur at that concurrency. As with §9.3's web leg, this is reported as
inconclusive-toward-collision-correctness rather than reached for as evidence of safety. **A second,
narrower caveat specific to the atomic arm's number:** the atomic arm dispatches run under `cucumber.js`'s
`default` profile (`retry: 1`), while the twin runs under `nonAtomicTwin` (`retry: 0`, deliberately, so a
retry can't mask the determinism signal — see `cucumber.js`'s own comment). TOM's raw per-scenario
telemetry (`metrics/raw/cucumber-jsonl/*.jsonl`) records only a scenario's final status, not attempt
count, so `scenario_outcome_history.csv` cannot itself distinguish "passed clean" from "passed after one
retry" for the atomic arm — the twin's 0.00 is retry-proof by construction, the atomic arm's is not.
Checked directly against the raw CI job logs rather than left as a caveat alone: all four atomic-web jobs'
`reads`/`writes` Cucumber summaries (`gh run view --job <id> --log`) report every scenario passing with no
`flaky`/`retry` text anywhere in the log, so no retry fired in this specific sweep — but that is a
per-run CI-log fact, not something the metrics pipeline itself can verify going forward.
...
- **Backend-load asymmetry contaminating the determinism instrument (found 2026-08-23, mitigated in
  tooling and verified not to have materialized).** The twin's precondition is a real UI login plus UI
  cart-building on every journey row; the atomic arm's precondition is a single API `$S_0$` call. Both
  arms hit the same shared backend, but the twin issues materially more requests per run to do it. This
  project has independently documented, twice in the same month and outside this evaluation, that
  concurrent CI load against the reference application's backend produces mid-run 502/503/429 responses,
  and that job-start staggering does not prevent mid-run collisions once a run is underway (`ci(helix)`
  commits `7c2079c`/`b9a3151`, and the ZAP Path Traversal false-positive investigation that reproduced
  the same load-correlated pattern). If this recurred during the 120-dispatch determinism campaign
  (§8.4), it would have raised the twin's measured pass↔fail transition rate for a backend-capacity
  reason, not a method reason — a direct threat to the instrument that carries the paper's central causal
  claim. The backend hosting plan was since upgraded from Render's free tier (verified via a subsequent
  all-33-jobs-green `platform=all` run, `32614000923`), which raises the load threshold before this
  triggers but does not remove the structural asymmetry itself — the twin still issues materially more
  requests per run than the atomic arm, so the confound is dormant, not eliminated by design.
  **Mitigated in tooling**: the campaign orchestrator (`run-campaign.ts`, built 2026-08-24) enforces
  strictly sequential dispatch — one `gh workflow run`, wait for terminal status, then the next — a
  hard concurrency cap of 1 forced by `ahm-execution-helix.yml`'s own `concurrency: cancel-in-progress`
  group, plus a disclosed `INFRASTRUCTURE_FAILURE`/`likelyInfra`-flagging rule that would have caught and
  excluded any load-correlated failure rather than silently folding it into the transition-rate count.
  **Verified, not just built**: the 120-dispatch determinism campaign (2026-08-29–31) and the 8-dispatch
  parallel-safety sweep (2026-08-24) both completed with **0 items flagged `likelyInfra`** — the confound
  did not manifest in either campaign's real data. The structural asymmetry itself is unchanged and could
  still resurface under a different load shape than either campaign exercised; reported as a mitigated,
  monitored, and empirically-checked risk, not a fully closed one.
