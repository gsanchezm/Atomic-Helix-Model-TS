...
**The non-atomic twin (`gate-twin-web`/`gate-twin-android`) lives in a separate workflow, `ahm-evaluation-campaign.yml`, split out of the Helix workflow on 2026-08-29.** Its two gates deliberately omit the `platform == 'all'` and `github.event_name != 'workflow_dispatch'` fallbacks every Helix gate has — they only fire on an explicit `platform: twin`, `twin-web`, or `twin-android` dispatch **against that file**, never against `ahm-execution-helix.yml`, and never as a side effect of `platform=all`. This is intentional, not an oversight: the twin is the Automated Atomic Testing paper's §8.4 evaluation-campaign artifact (`evaluation/non-atomic-twin/`, see `docs/paper/atomic-testing-formal-definition.md` §8.2–§8.4 and `docs/superpowers/specs/2026-07-23-atomic-testing-evaluation-campaign-design.md`), not part of the reference implementation's normal test surface. The split itself exists because both workflows previously shared one workflow-level `concurrency` group — an ordinary CI dispatch firing while the multi-day, strictly-sequential determinism/parallel-safety campaign (`scripts/experiments/run-campaign.ts`) was mid-flight would cancel the campaign's in-progress run outright, and vice versa. Each file now has its own independent group, so the two can never cancel each other. `eval-twin-web` runs the twin's K-row journey Outline under Playwright/Chromium and honors `cucumber_parallel` (the §8.4 parallel-safety sweep, 1/2/4/8); `eval-twin-android` runs it under Appium/Android at a fixed parallel=1 (single emulator — the parallel-safety sweep is web-only per the campaign design's own dispatch-count math). Both jobs are wired into the campaign orchestrator (`scripts/experiments/run-campaign.ts` / `pnpm experiments:run-campaign`), which dispatches strictly sequentially across both files — see that script's own header for why.
...
| `platform`     | Requires                                                                 | Dispatch inputs honored                                  |
|----------------|--------------------------------------------------------------------------|----------------------------------------------------------|
| `twin-web`     | `API_BASE_URL` + `BASE_URL`                                              | `cucumber_parallel` (§8.4 parallel-safety sweep)          |
