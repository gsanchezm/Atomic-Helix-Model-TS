...
| `ahm-evaluation-campaign.yml` | The Automated Atomic Testing paper's §3.2 non-atomic-twin evaluation artifact (`gate-twin-web`, `gate-twin-android`). Split out of `ahm-execution-helix.yml` on 2026-08-29 so the multi-day determinism/parallel-safety campaign's dispatches (`scripts/experiments/run-campaign.ts`) can never cancel — or be cancelled by — an ordinary CI run: each workflow has its own independent `concurrency` group. Manual dispatch only, via `platform: twin\|twin-web\|twin-android`. |
...
**Mobilewright, WebdriverIO and Cypress were removed from this workflow on 2026-07-30** — their jobs, gates, and `platform` dispatch values are gone (see `docs/superpowers/specs/2026-07-30-remove-mw-wdio-cypress-ci-design.md` for the rationale: all three categories failed across the board on their first-ever `platform=all` dispatch). Mobilewright and WebdriverIO plugin code, `docker-compose.yml` profiles, and `package.json` scripts are untouched by that removal — both tools remain usable locally; only their GitHub Actions execution is gone. **Cypress went further on 2026-08-21: the plugin (`src/plugins/cypress/`), its `package.json` scripts and dependencies, and every remaining CI/infra reference were removed from the project entirely.** Unlike the other plugins it never conformed to the kernel architecture — it ran outside `chaos-proxy` via its own runner, with duplicated step definitions instead of `ExecuteIntent` calls — so there was no local-usability case worth preserving.

A Jenkins declarative-pipeline equivalent of `ahm-execution-helix.yml` is available at the repo root as `Jenkinsfile` — container image pins and matrix dimensions translated to Jenkins' `matrix`/`lock()` DSL. Its job inventory has **not** been kept in sync with the 2026-07-30 removal above: the Jenkinsfile, like the other dormant CI configs (`.gitlab-ci.yml`, `azure-pipelines.yml`, both AWS buildspecs), still carries WebdriverIO/Mobilewright jobs that the GitHub Actions workflow no longer runs. None of these configs have ever executed, so this is recorded debt, not a live discrepancy. (Their Cypress jobs were deleted outright on 2026-08-21, alongside the plugin itself, rather than left as further debt.)
...
The Helix workflow gates jobs by the `platform` input on manual dispatch — 8 gates in total: `gate-api`, `gate-web-desktop`, `gate-web-responsive`, `gate-android`, `gate-ios`, `gate-gatling`, `gate-security`, `gate-a11y`, all following the identical `if: github.event_name != 'workflow_dispatch' || inputs.platform == 'all' || inputs.platform == '<name>'` pattern. There is no separate `zap`/`mobsf` dispatch value — `security-zap` and `security-mobsf` both gate on the single coarse `security` platform value. The `github.event_name != 'workflow_dispatch'` operand in that pattern reads as if a `push`/`pull_request` trigger would make every job run unconditionally — but the workflow's `on:` block currently has both triggers **commented out**: manual dispatch is the only way to trigger a run today, so every job stays scoped by whichever `platform` value you pass. (Re-enabling `push`/`pull_request` would restore that every-job-runs behavior verbatim; at that point, if you want mobile to stay manual-only, the gate `if:` blocks would need tightening to `github.event_name == 'workflow_dispatch' && …`.)

**The non-atomic twin (`gate-twin-web`/`gate-twin-android`) lives in a separate workflow, `ahm-evaluation-campaign.yml`, split out of the Helix workflow on 2026-08-29.** Its two gates deliberately omit the `platform == 'all'` and `github.event_name != 'workflow_dispatch'` fallbacks every Helix gate has — they only fire on an explicit `platform: twin`, `twin-web`, or `twin-android` dispatch **against that file**, never against `ahm-execution-helix.yml`, and never as a side effect of `platform=all`. This is intentional, not an oversight: the twin is the Automated Atomic Testing paper's §3.2.4 evaluation-campaign artifact (`evaluation/non-atomic-twin/`, see `docs/paper/atomic-testing-formal-definition.md` §3.2–§4 and `docs/superpowers/specs/2026-07-23-atomic-testing-evaluation-campaign-design.md`), not part of the reference implementation's normal test surface. The split itself exists because both workflows previously shared one workflow-level `concurrency` group — an ordinary CI dispatch firing while the multi-day, strictly-sequential determinism/parallel-safety campaign (`scripts/experiments/run-campaign.ts`) was mid-flight would cancel the campaign's in-progress run outright, and vice versa. Each file now has its own independent group, so the two can never cancel each other. `eval-twin-web` runs the twin's K-row journey Outline under Playwright/Chromium and honors `cucumber_parallel` (the §4.1 parallel-safety sweep, 1/2/4/8); `eval-twin-android` runs it under Appium/Android at a fixed parallel=1 (single emulator — the parallel-safety sweep is web-only per the campaign design's own dispatch-count math). Both jobs are wired into the campaign orchestrator (`scripts/experiments/run-campaign.ts` / `pnpm experiments:run-campaign`), which dispatches strictly sequentially across both files — see that script's own header for why.
...
**3. OmniPizza release must be published** (`gsanchezm/OmniPizza`) — required whenever a dispatch selects `android`, `ios`, `mobile`, `appium`/`appium-android`/`appium-ios`, or `security` (the resolver job also runs for `security`, since `security-mobsf` needs the platform binary too):
...
| `all`          | all of the above — the evaluation-campaign artifact (`ahm-evaluation-campaign.yml`) is a separate workflow, never included in `all`, see the note above | all of the above |

### Before you run `ahm-evaluation-campaign.yml`

Same secrets/variables as the Helix workflow (`API_BASE_URL`, `BASE_URL`, `GITHUB_TOKEN`) — this file has its own copy of `resolve-omnipizza-release` for the same OmniPizza-release prerequisite as `android`/`mobile` above, needed only for `twin`/`twin-android`.

```bash
gh workflow run ahm-evaluation-campaign.yml -f platform=twin-web -f cucumber_parallel=4
gh workflow run ahm-evaluation-campaign.yml -f platform=twin-android
gh workflow run ahm-evaluation-campaign.yml -f platform=twin   # both legs in one dispatch
```

| `platform`     | Requires                                                                 | Dispatch inputs honored                                  |
|----------------|--------------------------------------------------------------------------|----------------------------------------------------------|
| `twin-web`     | `API_BASE_URL` + `BASE_URL`                                              | `cucumber_parallel` (§4.1 parallel-safety sweep)          |
| `twin-android` | `API_BASE_URL` + OmniPizza release (`omnipizza-release.apk`)             | `android_api_level`                                       |
| `twin`         | union of `twin-web` + `twin-android`                                     | `cucumber_parallel`, `android_api_level`                   |

In practice these are dispatched by `scripts/experiments/run-campaign.ts` (`pnpm experiments:run-campaign`), not by hand — see that script's own header and the design doc for the full campaign orchestration story.
