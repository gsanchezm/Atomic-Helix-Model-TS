...
- [x] Results — the twin suites exist and are verified live on both platforms (see above). The campaign orchestrator (build-order step 5, `run-campaign.ts`/`aggregate-campaign-artifacts.ts`) was built and now drives §9.1 (parallel safety, `ps-2026-campaign`, 8/8 dispatches, populated 2026-08-31), §9.3 (determinism, `det-2026-campaign`, 120/120 dispatches, populated 2026-08-31), §9.4 (portability, populated 2026-08-23), and §9.5 (execution efficiency, N≥10 both platforms as of 2026-08-27/28). Only §9.2 (diagnosability) remains blocked — the fault-injection harness works locally (see the 2026-08-23 bullet above) but isn't wired into the orchestrator, so nothing dispatches it at scale yet.
