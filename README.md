...
| Workflow                    | Purpose                                                                  |
|-----------------------------|--------------------------------------------------------------------------|
| `ahm-execution-helix.yml`   | Unified test execution across every registered tool: API, web desktop/responsive (Playwright + visual), accessibility (axe), Android/iOS (Appium), security (ZAP + MobSF), Gatling perf, plus the Automated Atomic Testing paper's §8.4 non-atomic-twin evaluation artifact — 10 gated categories in total (`gate-api`, `gate-web-desktop`, `gate-web-responsive`, `gate-android`, `gate-ios`, `gate-gatling`, `gate-security`, `gate-a11y`, `gate-twin-web`, `gate-twin-android`). Manual dispatch only, via the `platform` input (`all` plus 21 values spanning API/web/mobile/perf/security/a11y/twin — see the platform table below for the full list) and `architecture_type: standard\|TOM` (TOM tags every artifact/record for the quantitative-architecture metrics pipeline and runs the extra `Consolidate` job). |
...
The Helix workflow gates jobs by the `platform` input on manual dispatch — 10 gates in total: `gate-api`, `gate-web-desktop`, `gate-web-responsive`, `gate-android`, `gate-ios`, `gate-gatling`, `gate-security`, `gate-a11y`, `gate-twin-web`, `gate-twin-android`. The first 8 follow the identical `if: github.event_name != 'workflow_dispatch' || inputs.platform == 'all' || inputs.platform == '<name>'` pattern. There is no separate `zap`/`mobsf` dispatch value — `security-zap` and `security-mobsf` both gate on the single coarse `security` platform value. The `github.event_name != 'workflow_dispatch'` operand in that pattern reads as if a `push`/`pull_request` trigger would make every job run unconditionally — but the workflow's `on:` block currently has both triggers **commented out**: manual dispatch is the only way to trigger a run today, so every job stays scoped by whichever `platform` value you pass. (Re-enabling `push`/`pull_request` would restore that every-job-runs behavior verbatim; at that point, if you want mobile to stay manual-only, the gate `if:` blocks would need tightening to `github.event_name == 'workflow_dispatch' && …`.)

**`gate-twin-web`/`gate-twin-android` are the two exceptions** — they deliberately omit the `platform == 'all'` and `github.event_name != 'workflow_dispatch'` fallbacks that every other gate has. They only fire on an explicit `platform: twin`, `twin-web`, or `twin-android` dispatch. This is intentional, not an oversight: the twin is the Automated Atomic Testing paper's §8.4 evaluation-campaign artifact (`evaluation/non-atomic-twin/`, see `docs/paper/atomic-testing-formal-definition.md` §8.2–§8.4 and `docs/superpowers/specs/2026-07-23-atomic-testing-evaluation-campaign-design.md`), not part of the reference implementation's normal test surface — an ordinary `platform=all` (or a future re-enabled `push`/`pull_request` trigger) must never accidentally fire a campaign dispatch. `eval-twin-web` runs the twin's K-row journey Outline under Playwright/Chromium and honors `cucumber_parallel` (the §8.4 parallel-safety sweep, 1/2/4/8); `eval-twin-android` runs it under Appium/Android at a fixed parallel=1 (single emulator — the parallel-safety sweep is web-only per the campaign design's own dispatch-count math). Neither job is wired into a campaign orchestrator yet — that (driving both jobs across the full ~156-dispatch matrix, with resumability and a concurrency cap) is tracked separately as build-order step 5 in the campaign design spec.
...
**3. OmniPizza release must be published** (`gsanchezm/OmniPizza`) — required whenever a dispatch selects `android`, `ios`, `mobile`, `appium`/`appium-android`/`appium-ios`, `security`, `twin`, or `twin-android` (the resolver job also runs for `security`/`twin`/`twin-android`, since `security-mobsf`/`eval-twin-android` need the platform binary too):
...
gh workflow run ahm-execution-helix.yml -f platform=twin-web -f cucumber_parallel=4
gh workflow run ahm-execution-helix.yml -f platform=twin-android
```
...
| `all`          | all of the above **except `twin`/`twin-web`/`twin-android`** — the evaluation-campaign artifact is never included in `all`, see the `gate-twin-*` note above | all of the above |
