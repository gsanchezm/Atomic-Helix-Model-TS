...
**3. OmniPizza release must be published** (`gsanchezm/OmniPizza`) — required because mobile runs on every push/PR (see note above), and now also by `e2e-mobilewright` (subject to the known device-passport gap noted above) and `security-mobsf` (downloads both platform binaries fresh every run for its static scan):

- The repo's `releases/latest` must exist with assets named **exactly** `omnipizza-release.apk` (Android) and `OmniPizza-Simulator.zip` (iOS). The resolver job reads `tag_name` from `/releases/latest`; the mobile jobs (including `e2e-mobilewright` and `security-mobsf`) download `<base_url>/<asset>`.
